...
export async function SiteHeader() {
  ...
  return (
    <header className="sticky top-0 z-40 border-b border-white/12 bg-slate-950/90 backdrop-blur-xl">
      <div className="mx-auto flex max-w-7xl items-center justify-between gap-4 px-6 py-4 text-white">
        ...
        <nav className="hidden items-center gap-1 rounded-full border border-white/12 bg-white/[0.07] p-1 md:flex">
          {publicNav.map((item) => (
            <Link
              key={item.href}
              href={item.href}
        ...
        <div className="flex items-center gap-2 rounded-full border border-white/12 bg-white/[0.07] p-1.5">
